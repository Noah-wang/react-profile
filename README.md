...
### 6. 3D地球展示组件
- **Three.js 集成**：使用 Three.js 实现 3D 地球模型
- **实时旋转**：基于时间自动旋转的地球效果
- **交互控制**：支持鼠标拖拽和滚轮缩放
- **性能优化**：使用 WebGL 渲染和帧率控制
- **响应式适配**：自适应不同屏幕尺寸的显示效果

### 7. 卡片翻转效果
- **3D 变换**：使用 CSS transform-style 实现真实的 3D 翻转
- **透视效果**：通过 perspective 属性创建深度感
- **状态管理**：使用 React state 控制翻转状态
- **动画过渡**：平滑的翻转动画和阴影效果
- **内容切换**：前后两面内容的无缝切换

### 8. 粒子动画系统
- **Canvas 渲染**：使用 HTML5 Canvas 实现高性能粒子效果
- **物理模拟**：基于物理引擎的粒子运动模拟
- **交互响应**：鼠标移动影响粒子行为
- **性能优化**：使用 requestAnimationFrame 和粒子池
- **自适应系统**：根据设备性能调整粒子数量
